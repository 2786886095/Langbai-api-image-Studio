...
const CODEX_GATEWAY_LOCALES = Object.freeze({
  "zh-CN": Object.freeze({
    ...
    codexGatewayQueueHint: "可输入 1–100，默认 5；数值越高，对账号会话与网络稳定性的要求越高。",
    codexGatewayFacts: "GPT Image 2 · 最多 20 张参考图 · ChatGPT 网页会话 · 异步任务最长轮询 20 分钟",
    codexGatewayCapability: "最大并发可在 1–100 间自行调整，默认 5；成功、失败和取消后都会清理临时对话。",
  ...
  "zh-Hant": Object.freeze({
    ...
    codexGatewayQueue: "同時生成數", codexGatewayQueueHint: "可輸入 1–100，預設 5；批次工作會重用帳號工作階段並自動清理臨時對話。",
    codexGatewayFacts: "GPT Image 2 · 最多 20 張參考圖 · Bearer 下載 · 非同步任務最長輪詢 20 分鐘",
    codexGatewayCapability: "並發限制為 1–100，預設 5；成功、失敗與取消後都會清理臨時對話。",
  ...
  en: Object.freeze({
    ...
    codexGatewayQueue: "Concurrent generations", codexGatewayQueueHint: "Enter 1–100; the default is 5. Higher values require a more stable account session and network.",
    codexGatewayFacts: "GPT Image 2 · up to 20 references · ChatGPT web session · async task polling up to 20 minutes",
    codexGatewayCapability: "Maximum concurrency is adjustable from 1–100 with a default of 5; temporary chats are cleaned after success, failure, and cancellation.",
  ...
  ja: Object.freeze({
    ...
    codexGatewayQueue: "同時生成数", codexGatewayQueueHint: "1～100 を入力でき、既定値は 5 です。バッチ処理はアカウントセッションを再利用し、一時会話を自動削除します。",
    codexGatewayFacts: "GPT Image 2 · 参照最大 20 枚 · Bearer ダウンロード · 非同期タスクを最大 20 分間ポーリング",
    codexGatewayCapability: "同時生成数は 1～100、既定値は 5 です。成功・失敗・キャンセル後に一時会話を削除します。",
  ...
  ko: Object.freeze({
    ...
    codexGatewayQueue: "동시 생성 수", codexGatewayQueueHint: "1–100을 입력할 수 있으며 기본값은 5입니다. 일괄 작업은 계정 세션을 재사용하고 임시 대화를 정리합니다.",
    codexGatewayFacts: "GPT Image 2 · 참고 이미지 최대 20장 · Bearer 다운로드 · 비동기 작업 최대 20분 폴링",
    codexGatewayCapability: "동시 생성 수는 1–100, 기본값은 5입니다. 성공, 실패, 취소 후 임시 대화를 정리합니다.",
...
function getCodexGatewayConcurrency(options = getCodexGatewayOptions()) {
  return Math.max(1, Math.min(
    ...
    Math.floor(Number(options.clientQueue) || 5),
      ...
      try {
        ...
        if (shouldSwitchChatGptAccount(normalized) && chatGptAccountsState.auto_switch !== false) {
          ...
          const rotated = await nativeDownload.rotateChatGptAccount(
            ...
            reason,
            !refreshedAccounts.has(account.local_account_id),
          );
          renderChatGptAccounts(rotated);
          if (rotated?.session_refreshed && rotated?.rotated_to) {
            refreshedAccounts.add(rotated.rotated_to);
            attemptedAccounts.delete(rotated.rotated_to);
            showStatus("ChatGPT 网页会话已自动刷新，正在重试当前图片", "warning");
            continue;
          }
...
const nativeDownload = (() => {
  ...
  return {
    ...
    rotateChatGptAccount(failedStatus, reason, allowSessionRefresh = true) {
      return request("rotateChatGptAccount", { failedStatus, reason, allowSessionRefresh }, 90000);
    },
