...
const CODEX_GATEWAY_LOCALES = Object.freeze({
  "zh-CN": Object.freeze({
    ...
    codexGatewayQueueHint: "可输入 1–3，默认 3；批量任务会复用账号会话并自动清理临时对话。",
    codexGatewayFacts: "GPT Image 2 · 最多 20 张参考图 · ChatGPT 网页会话 · 异步任务最长轮询 20 分钟",
    codexGatewayCapability: "并发限制为 1–3，默认 3；降低批量任务对账号会话的冲击，成功、失败和取消后都会清理临时对话。",
  ...
  "zh-Hant": Object.freeze({
    ...
    codexGatewayQueue: "同時生成數", codexGatewayQueueHint: "可輸入 1–3，預設 3；批次工作會重用帳號工作階段並自動清理臨時對話。",
    codexGatewayFacts: "GPT Image 2 · 最多 20 張參考圖 · Bearer 下載 · 非同步任務最長輪詢 20 分鐘",
    codexGatewayCapability: "並發限制為 1–3，預設 3；成功、失敗與取消後都會清理臨時對話。",
  ...
  en: Object.freeze({
    ...
    codexGatewayQueue: "Concurrent generations", codexGatewayQueueHint: "Enter 1–3; the default is 3. Batch jobs reuse account sessions and clean up temporary chats.",
    codexGatewayFacts: "GPT Image 2 · up to 20 references · ChatGPT web session · async task polling up to 20 minutes",
    codexGatewayCapability: "Concurrency is limited to 1–3 with a default of 3; temporary chats are cleaned after success, failure, and cancellation.",
  ...
  ja: Object.freeze({
    ...
    codexGatewayQueue: "同時生成数", codexGatewayQueueHint: "1～3 を入力でき、既定値は 3 です。バッチ処理はアカウントセッションを再利用し、一時会話を自動削除します。",
    codexGatewayFacts: "GPT Image 2 · 参照最大 20 枚 · Bearer ダウンロード · 非同期タスクを最大 20 分間ポーリング",
    codexGatewayCapability: "同時生成数は 1～3、既定値は 3 です。成功・失敗・キャンセル後に一時会話を削除します。",
  ...
  ko: Object.freeze({
    ...
    codexGatewayQueue: "동시 생성 수", codexGatewayQueueHint: "1–3을 입력할 수 있으며 기본값은 3입니다. 일괄 작업은 계정 세션을 재사용하고 임시 대화를 정리합니다.",
    codexGatewayFacts: "GPT Image 2 · 참고 이미지 최대 20장 · Bearer 다운로드 · 비동기 작업 최대 20분 폴링",
    codexGatewayCapability: "동시 생성 수는 1–3, 기본값은 3입니다. 성공, 실패, 취소 후 임시 대화를 정리합니다.",
