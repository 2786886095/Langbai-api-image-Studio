# Claude Handoff: AI 图片生成器 v1.2.9

更新时间：2026-07-03
项目路径：`F:\AI\agent\图像生成`
GitHub：`https://github.com/2786886095/Langbai-api-image-Studio`
当前 Release：`https://github.com/2786886095/Langbai-api-image-Studio/releases/tag/v1.2.9`

**没有 v1.2.8 的 Release**：v1.2.8（模型列表改成下拉框）commit 已推送、CI 也过了，但发布验证过程中用户就已经发现了下面这个 flex-shrink bug，所以没有创建 v1.2.8 的 GitHub Release，直接把修复并入 v1.2.9 一起发布。如果看到 git log 里有 v1.2.8 的 commit 但 GitHub Releases 里没有对应 tag，这是预期状态，不是遗漏。
...
用户反馈"模型选择列表也跟别的列表做的一样啊，现在做的太难用了"——`#modelChoices`（检测到的模型列表）从 v1.2.2 起一直是常驻展开的按钮网格（`overflow:auto` 内部滚动），跟其他 6 个已经改成下拉框的字段风格不一致，且正是好几轮"滚轮误滚"报告的直接来源。**v1.2.8** 把它改成和其他 6 个一模一样的 `.custom-select` 组件：隐藏原生 `<select id="modelChoices">` 做状态容器，配 `#modelChoicesTrigger` 按钮 + `#modelChoicesCustomList` 弹出列表，选中后通过 `change` 监听把值同步到 `#model` 文本框（跟原来点按钮的行为一致）。改完之后它自动复用通用的 `resolveWheelEventStartElement` 滚轮修复，不用再单独处理。

**v1.2.8 commit 推送后、还没创建 GitHub Release 之前**，用户就带截图反馈"模型列表都重叠到一起了，根本看不清，也选不了"。**这次务必先复现再下结论**：前几轮全是 webview_windows 的锅，这次先入为主也怀疑是同一类问题，但用 CDP 在 headless Edge 里拿真实的 13 个 GrsAI 模型直接复现出了一模一样的重叠——**证明这是自己代码的真 bug，不是插件问题**。

**根因**：`.custom-select-list` 是 `display:flex; flex-direction:column; max-height:240px; overflow-y:auto`，子项 `.custom-select-option` 没有设置 `flex-shrink`（默认值 `1`，允许压缩）。当内容总高度超过 `max-height` 时，flexbox 的行为是先按 `flex-shrink` 把所有子项压缩到能塞进容器里，压缩没法完全解决才轮到 `overflow-y:auto` 生效——不是像很多人以为的"容器直接滚动，子项保持原尺寸"。压缩后每一项的实际渲染高度（16px）比它自己的 `padding: 8px 10px`（光 padding 就要 16px）还小，文字必然溢出、糊到相邻行上。用 CDP 读 `getComputedStyle` 直接量化确认过：`offsetHeight: 16` vs 实际需要的 `scrollHeight: 36`。

**这是所有下拉框共享的潜在 bug**，v1.2.2 那 6 个理论上都有同样的风险，只是模型列表最容易先踩到——真实 API 返回的模型经常有 10+ 个，而"已保存的 API"/"常用尺寸"这些用户很少存那么多条，所以从来没触发过。

**修复**（`style.css`）：给 `.custom-select-option` 加一行 `flex-shrink: 0`，让内容多的列表老老实实交给 `overflow-y:auto` 处理，而不是被压缩变形。

**验证方法**（这次做得比较完整，值得当模板复用）：
1. 用 `document.getElementById('modelChoices').options` populate 13+ 真实模型，在 headless Edge 里截图，直接复现出跟用户截图一样的重叠——排除"只有真机 exe 才复现"的可能性，缩小问题范围。
2. 写一个诊断脚本读 `getBoundingClientRect()` + `getComputedStyle()`，发现 DOM 布局位置本身没重叠（每项 `top` 间隔 18px，符合预期），但 `offsetHeight`（16px）远小于 `scrollHeight`（36px）——定位到是"内容被压缩裁切"而不是"定位错误"。
3. 改完 `flex-shrink:0` 后重新截图 + 重新跑诊断脚本，确认 `offsetHeight === scrollHeight`（36px），视觉上完全清晰。
4. 回归测试新增断言，检查所有 option 的 `offsetHeight >= scrollHeight`；用 `git stash` 只挡住这一行 CSS 改动重新跑测试，确认没有修复时是 `41/41` 项被压缩，验证了这条新断言真的在测这个 bug，不是巧合通过。
...
v1.0.11 → v1.2.0 是用户明确指定的跳跃，不代表中间有跳过的版本。之后 v1.2.1 → v1.2.9 都是常规 +0.0.1 递增（v1.2.4~v1.2.9 是同一会话里连续排查出的独立问题各自单独发版，不是攒了多个改动一起发；v1.2.5/v1.2.6/v1.2.7 是同一个滚轮 bug 三次不同角度的补强；v1.2.9 是 v1.2.8 发布验证阶段发现问题后的直接修复，v1.2.8 本身没有创建 Release）。以后如果用户没有明确指定版本号，继续按 +0.0.1 递增；如果用户明确报了一个版本号，直接照用户说的来。

## 当前核心状态

- 应用版本：`APP_VERSION = "1.2.9"`；`pubspec.yaml` 为 `1.2.9+22`
- 前端缓存/query：`index.html` 中 `20260703-1-2-9`；Service Worker cache：`ai-image-generator-1-2-9-20260703`
- Windows 版本资源：`windows/runner/Runner.rc` 兜底值 `1,2,9,22`/`"1.2.9"`
- Android assets 已与根目录 Web 文件同步（SHA256 一致校验过）
- v1.2.3~v1.2.7、v1.2.9 均已创建 GitHub Release（各自附 SHA256SUMS.txt）；v1.2.8 只有 commit+CI 通过，没有 Release（见文档开头说明）
...
- v1.2.7 的 `elementFromPoint` 坐标纠正修复已用"故意构造错误 target、正确坐标"的方式验证过回归测试真的在测这个 bug（`git stash` 挡住修复重跑确认会失败、恢复后确认变绿），但**仍然没有真机 Windows exe 用真实鼠标滚轮做端到端验证**（CDP 能验证的是"代码逻辑对不对"，验证不到真实 `webview_windows` 插件在各种硬件/滚轮设备上具体会把 `event.target`/坐标报成什么样）。如果用户继续反馈类似问题，先确认安装的是不是 v1.2.9 或更新版本。
- v1.2.9 的 `flex-shrink:0` 修复已经在 headless 浏览器里用真实的 13 个模型完整复现问题+验证修复，不依赖真机 exe，可信度比滚轮那几轮更高。
...
- **不是所有"只有打包后才复现"的问题都是 webview_windows 的锅**：v1.2.8 的模型下拉框重叠问题一开始也怀疑是插件问题，但用 headless Edge 直接复现出了一样的现象，证明是自己的 CSS bug（`.custom-select-option` 缺 `flex-shrink:0`）。**每次遇到"exe 端才有问题"的报告，先花两分钟在普通浏览器里用真实数据量复现一次，不要因为最近几次都是同一个原因就跳过这一步。**
- **`display:flex; flex-direction:column` 容器配 `max-height`+`overflow-y:auto` 时，子项必须显式设置 `flex-shrink:0`**，否则内容超出 `max-height` 时 flexbox 会把所有子项压缩到塞进容器里（而不是让容器滚动），压缩到比子项自身 padding 还小会导致内容溢出/视觉重叠。`.custom-select-option` 已经加了这个规则；以后新增其他类似"弹性列表+固定高度+溢出滚动"的组件时要记得同步加上。
...
- 不要把 `#modelChoices` 改回常驻展开的按钮网格；不要删掉 `.custom-select-option` 的 `flex-shrink: 0`。
...
- 找用户确认 v1.2.9 的滚轮修复、模型下拉框和 v1.2.3 的 folder-save 功能在真机上表现是否符合预期。
