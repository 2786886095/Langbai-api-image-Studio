# Claude Handoff: AI 图片生成器 v1.3.4

更新时间：2026-07-04
项目路径：`F:\AI\agent\图像生成`
GitHub：`https://github.com/2786886095/Langbai-api-image-Studio`
当前 Release：`https://github.com/2786886095/Langbai-api-image-Studio/releases/tag/v1.3.4`
...
- 应用版本：`APP_VERSION = "1.3.4"`；`pubspec.yaml` 为 `1.3.4+28`
- 前端缓存/query：`index.html` 中 `20260704-1-3-4`；Service Worker cache：`ai-image-generator-1-3-4-20260704`
- Windows 版本资源：`windows/runner/Runner.rc` 兜底值 `1,3,4,28`/`"1.3.4"`
- Android assets 已与根目录 Web 文件同步（SHA256 一致校验过）
- v1.2.3~v1.2.7、v1.2.9、v1.2.10、v1.3.0、v1.3.1、v1.3.2、v1.3.3 均已创建 GitHub Release；v1.2.8 只有 commit+CI 通过，没有 Release；**v1.3.4 截至本次交接是否已推送/发布取决于用户对推送的确认，见下方"下一步建议"**
...
- **最优先**：找用户确认 v1.3.3 的 fork 依赖切换（`-32000` 坐标偏移）是否真的解决了"移动窗口后原位置桌面区域点不了"的问题——这是本次修复完全没有真机验证过的部分（本机无 VS 工具链、也没有条件手动复现）。如果用户反馈问题依旧，说明这次的根因诊断/修复也不对症，需要重新去查 `windows/webview.cc` 里是否还有其它地方也用了类似的硬编码屏幕坐标逻辑，或者去查 webview_windows 的 issue 列表里有没有更贴切的报告。
...
- **有三件事用户明确要求"先说清楚需求再动手"，v1.3.4 没有包含，需要接手后继续跟进澄清**：
  1. 更新提醒不够醒目——已经查明启动时的全屏确认弹窗逻辑本身没问题（`checkForUpdatesOnLaunch`，只要 GitHub 有更新的 Release 就会弹），用户之前没看到纯粹是因为当时确实没有比 v1.3.1 更新的 Release（v1.3.2/v1.3.3 都还没发布），v1.3.3 发布后这个问题可能已经自然消失，需要用户确认重新启动后是否看到了弹窗，如果看到了就不需要再改代码。
  2. 用户对更新提醒还提过"想要更持久的提示"这个方向（不只是启动一次性弹窗），具体想要什么形式（常驻顶部横幅？设置里的红点？）还没问清楚。
  3. 新的"嵌字模式"功能（给每张参考图分别上传+加文字气泡，绕开一次性上传太多图导致 HTTP 413）——这是本次会话里最大的一块未澄清需求，关键分歧在于"气泡文字是让 AI 模型画上去（发提示词），还是本地用 Canvas 直接叠加图层（不调用 AI，完全独立于现有生成流程）"，两条路线实现方式完全不同，**不要凭感觉选一个开始写，一定要先问清楚**。
- 用户此前有一条尚未处理的陈述："还有图片其实已经缓存在本地，默认保存在setup安装的outputs里面，用户也可自行在软件内保存图片和压缩包到其它路径"——发在 v1.3.2 修复过程中，用户当时用"继续"打断了我的澄清提问，意图不明确（是要确认现状、报 bug、还是要求新功能？），至今没有回头处理。接手后如果用户没有主动重提，不要臆测着直接改代码，应该先追问清楚具体诉求。
