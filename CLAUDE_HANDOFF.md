# Claude Handoff: AI 图片生成器 v1.3.22

更新时间：2026-07-17
项目路径：`F:\AI\agent\图像生成`
GitHub：`https://github.com/2786886095/Langbai-api-image-Studio`
本交接对应源码版本：`v1.3.22+46`；线上发布状态以 GitHub Releases 实际页面为准。

## v1.3.22 当前状态（优先读）

`v1.3.22` 移除漫画分镜和嵌字批量提示词的 100 条硬上限：漫画按输入行数自动扩展，嵌字允许导入超过 100 张图片并逐行对应；安全上仍保留嵌字图片总大小和全局参考图数量限制。105 条专项回归已同步，详细发布清单见根目录 `CODEX_HANDOFF.md`。

## v1.3.21 当前状态

`v1.3.21` 新增独立生成图片临时缓存：无论是否开启历史，成功图片都会立即持久缓存，默认保留 7 天且可在设置中调整或立即清理；历史直接复用 `cache://` 字节，生成阶段不会写入用户目录，只有 ZIP/保存到文件夹才正式落盘。五语言和缓存专项回归已同步，详细发布清单见根目录 `CODEX_HANDOFF.md`。

## v1.3.20 当前状态

`v1.3.20` 修复“全部失败重试”偶发无响应：重试时工具栏不再因失败数归零而消失，按钮可取消整轮重试；取消会中断挂起请求、恢复失败卡、释放锁，并允许立即重新重试。五语言和挂起→取消→再次成功的回归用例已同步。详细发布清单见根目录 `CODEX_HANDOFF.md`。

## v1.3.19 当前状态

`v1.3.19` 为漫画分镜与嵌字模式新增“批量输入提示词”：每行严格对应一个分镜或一张按名称排序的图片，保留空行位置；漫画可自动补足分镜，嵌字数量溢出会阻止写入，覆盖已有内容需要确认。五语言与回归测试已同步。详细发布清单见根目录 `CODEX_HANDOFF.md`。
