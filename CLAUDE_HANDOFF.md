# Claude Handoff: AI 图片生成器 v1.3.15

更新时间：2026-07-06
项目路径：`F:\AI\agent\图像生成`
GitHub：`https://github.com/2786886095/Langbai-api-image-Studio`
当前 Release：`https://github.com/2786886095/Langbai-api-image-Studio/releases/tag/v1.3.15`（截至本次交接是否已发布取决于用户对推送的确认，见下方"下一步建议"）
...
- 应用版本：`APP_VERSION = "1.3.15"`；`pubspec.yaml` 为 `1.3.15+39`
- 前端缓存/query：`index.html` 中 `20260706-1-3-15`；Service Worker cache：`ai-image-generator-1-3-15-20260706`
- Windows 版本资源：`windows/runner/Runner.rc` 兜底值 `1,3,15,39`/`"1.3.15"`
- Android assets 已与根目录 Web 文件同步（SHA256 一致校验过）
- v1.2.3~v1.2.7、v1.2.9、v1.2.10、v1.3.0~v1.3.14 均已创建 GitHub Release；v1.2.8 只有 commit+CI 通过，没有 Release；**v1.3.15 截至本次交接是否已推送/发布取决于用户对推送的确认，见下方"下一步建议"**
- **Windows 端应用内更新现在会显式传 `/DIR=<当前运行 exe 所在目录>` 给 Setup.exe（见"v1.3.15"一节）**——`_downloadWindowsUpdate()` 用 `File(Platform.resolvedExecutable).parent.path` 算出这个值，不再依赖 Inno Setup 自己"沿用上次安装目录"的注册表探测（那条内置机制在某些情况下会静默失效，退回 `DefaultDirName={localappdata}`，导致用户装在别的盘时更新反而在 C 盘另起一份）。这处改动没有自动化回归测试覆盖，也没做过真机端到端验证，见"已知未验证/延后事项"。
...
- **v1.3.15 的 Windows 更新安装路径修复（`/DIR=` 显式传参）没有做过真机端到端验证**——本机没有条件复现"用户在自定义路径装好旧版本 → 触发应用内静默更新 → 确认新版本原地覆盖、C 盘没有多出一份"这个完整链路，只验证了 `flutter analyze` 干净、代码逻辑本身合理。**这是当前优先级最高的待验证项**：接手后如果用户能确认已经更新到这一版，应该主动问一句"这次更新之后，安装路径还是不是你自己选的那个（比如 F:\AI\picture\...），C 盘有没有又多出一份"，这是唯一能确认这处修复真的解决问题的方式。
...
- 不要把 `_downloadWindowsUpdate()` 里的 `/DIR=$installDir` 参数删掉、也不要改回"什么都不传，指望 Inno Setup 自己探测上次安装目录"——那条内置探测机制依赖注册表记录，会在某些情况下静默失效并退回 `DefaultDirName`（C 盘），这正是 v1.3.15 要修的问题；`installDir` 必须继续用 `Platform.resolvedExecutable` 的父目录动态算，不要写死成某个固定路径（见"v1.3.15"一节）。

## 下一步建议

- **最优先**：找用户确认 v1.3.15 的更新安装路径修复是否真的生效——更新之后新版本是不是原地覆盖在用户自己选的路径（比如 F:\AI\picture\AI Image Generator），C 盘 `%LOCALAPPDATA%\AI Image Generator` 有没有又多出一份。这处修复没有做过真机端到端验证，是当前最需要用户实测反馈的一项。
