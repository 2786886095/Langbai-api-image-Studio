...
## v1.3.10：生图原生调用超时从 5 分钟延长到 15 分钟 + "全部失败重试"加进度条反馈

用户反馈两个连在一起的问题："现在显示的反而不是400 总是原生调用超时 取消调用时间设定 还有全部失败重试依然经常点不动 用不了"。排查后发现这两个症状同源，值得完整记录。
...
"全部失败重试依然经常点不动 用不了"跟上面是同一个根因的另一个表现：`retryAllFailedResults()` 对每张失败卡片重新调用 `retryResultCard()` → `callImageAPI()` → 最终还是 `smartFetch()`，如果某几张卡片的重试又撞上原生超时，每一张都要先干等满整个超时时长才会失败结算，而这段时间里"全部失败重试"按钮除了显示一个从点击起就没再变过的"重试中 (N)"文字之外，没有任何进度反馈——button 被禁用几分钟、完全看不出还在正常工作还是已经卡死，体感自然是"点不动、用不了"（`retryAllFailedResults()` 本身在结构上不会真正死锁：`concurrentLimitSettled()` 内部每个任务都用 `.then/.catch` 包过，保证 `Promise.all` 一定会等到所有任务真正 settle 才返回，而每个任务最终都会在 JS 侧超时到点时被迫 reject——所以"卡死"其实是"合理但漫长的等待+零反馈"造成的观感，不是真的挂起）。

### 修法

1. **`smartFetch()` 里两处 `nativeDownload.nativeFetchPayload(payload, ...)` 调用的超时从 `5 * 60 * 1000` 改成 `15 * 60 * 1000`**——15 分钟，跟 `chooseDir` 用的时长一致（这个代码库里"这个操作可能确实要很久，给够时间"的既有约定）。**没有做成真正的"取消超时"（比如传 `Infinity`）**：原生侧确认没有自己的读超时兜底，真的彻底不设上限的话，一旦某次原生调用真的永久卡死（比如连接半开但既不来数据也不断开），`pending` 里那条记录会永远留着，且没有任何恢复手段（连"停止生成"都够不着它，因为 `nativeDownload.request()` 目前根本不接受/不响应 AbortSignal）。15 分钟远超任何正常生图请求应该花的时间，实际体验上跟"没有超时"没有区别，但保留了一个绝对意义上的兜底。
2. **`GRSAI_MAX_POLL_COUNT` 从 180 改成 450**（配合原有 `GRSAI_POLL_INTERVAL_MS = 2000` 不变，轮询总时长从 6 分钟延长到 15 分钟）——GrsAI 走"提交任务拿 ID + 轮询 `/v1/api/result`"这条异步路径时，自己有一套独立的轮询次数上限，跟上面的原生调用超时是两回事，但本质上是同一类"生成时间上限"设置，一并延长到 15 分钟保持前后一致。
3. **`retryAllFailedResults()` 加上进度条反馈**：复用批量生成本来就有的 `#progressWrap`/`updateProgress(done, total, icon)`（`generateComic()`/`generateCaptions()` 早就在用这套 UI，只有"全部失败重试"这条路径没接），点击后立刻显示进度条并从 `0/N` 开始，每张卡片重试结算后（无论成功失败）递增 `done` 并刷新进度条，全部完成后维持"✅ N/N"3 秒再自动隐藏（跟批量生成结束后的隐藏节奏一致）。这不改变重试的并发方式或重试次数逻辑，纯粹是让一个可能要等很久的操作在等待过程中"看得见在动"，而不是显得像卡死。

### 没有做但要知道的事

- **没有给 `nativeDownload.request()` 加真正的取消（AbortSignal）支持**：目前"停止生成"按钮对一个已经发给原生层、还没返回的 `nativeFetch` 调用没有任何作用，只能等它自己超时或返回。这是一个更大的架构改动（需要原生侧也支持"根据 id 主动砍断某个在途请求"，Android/Windows 两条桥接路径都要改），这次没有做，仅仅是把超时从 5 分钟放宽到 15 分钟、并让"重试全部失败"的等待过程可见。如果以后用户明确要求"点停止后要能立刻打断已经发出去的请求"，需要单独排期做这件事。
- **没有把原生超时错误纳入 `isTransientApiError()`/`retryTransient()` 的自动重试范围**：故意不做——那套机制目前只认 HTTP 400，一次原生超时本身就已经等了 15 分钟，如果再自动重试（乘以用户设置的重试次数），单张图片失败前的等待时间会成倍増加，体验只会更差，不会更好。
- **超时数值的改动无法在 `qa/regression-runner.js` 的 headless Edge 环境里自动化验证**：CDP 测试跑在普通浏览器里，没有 `FlutterDownload` 桥接对象，`nativeDownload.available()` 恒为 false，`smartFetch()` 只会走普通 `fetch()`，压根不会触发这条超时逻辑——这跟 v1.3.2 延长到 5 分钟时是同样的处境。这次真正加了自动化回归覆盖的是"重试全部失败"的进度条行为本身（`testRetryClearReloadAndI18n` 里新增的 `progressVisibleDuringRetry`/`progressReachedTotal`/`progressHiddenAfterDelay` 三个断言，通过给 mock fetch 加错开的延迟让中间状态可观察），超时数值的改动只能靠真机长时间生成一次验证。
...
- **生图相关的原生调用超时是 15 分钟，不是 5 分钟（见"v1.3.10"一节）**——`smartFetch()` 里两处 `nativeFetchPayload` 调用、`GRSAI_MAX_POLL_COUNT`（450，配合 2 秒间隔）都已同步改成 15 分钟口径。不要因为"看起来很长"就往回调；原生侧（`_nativeFetch()`）没有自己的读超时，这个 JS 侧数值是唯一的兜底，5 分钟被证实对一部分同步阻塞式生成接口不够用。
...
- **v1.3.2 把 `nativeFetchPayload` 超时延长到 5 分钟，后来被 v1.3.10 证实不够用，已经改成 15 分钟**（见"v1.3.10"一节）——用户实际使用后反馈频繁看到"原生功能调用超时"，说明确实有请求正常运行超过了 5 分钟。v1.3.10 的 15 分钟同样**没有真机验证过"生图请求真的跑到 10-15 分钟"的场景**，只是根据用户反馈的方向做了更大幅度的延长；如果用户之后还是反馈同样的超时错误，说明 15 分钟可能仍然不够，或者存在真正的原生调用挂起（不是"慢"而是"卡死"），需要考虑给 `nativeDownload.request()` 加真正的 AbortSignal 取消支持（v1.3.10 没做，见该节"没有做但要知道的事"）而不是继续一味调大数字。
...
- **v1.3.10 的 15 分钟超时延长本质上无法在这台机器上真机验证**（需要真的等一个跑 10+ 分钟的生图请求，本机没有条件触发这种慢请求）——这次真正验证扎实的是"重试全部失败"的进度条行为本身（headless Edge + `git stash` 双向验证新断言会失败/通过），超时数值改动纯粹是根据用户反馈方向做的合理性判断，**这是当前优先级最高的待验证项**：接手后如果用户能提供一个会话导致长时间生成的具体场景（哪个模型、哪个供应商），应该主动找用户实测确认新的 15 分钟是否真的够用、以及"全部失败重试"点击后进度条是否符合预期。
...
- 不要把 `chooseDir` 的超时改回默认的 120 秒；不要把 `smartFetch()` 里两处 `nativeFetchPayload` 调用的超时改回 5 分钟或默认值——现在是 15 分钟（v1.3.10，见该节根因排查），5 分钟已经被用户实际反馈证实不够用。同理不要把 `GRSAI_MAX_POLL_COUNT` 改回 180，现在是 450（配合 `GRSAI_POLL_INTERVAL_MS = 2000` 凑够 15 分钟，跟上面的原生超时口径保持一致）。
...
- 不要从 `retryAllFailedResults()` 里删掉 `dom.progressWrap`/`updateProgress()` 调用改回纯按钮文字反馈——15 分钟超时下，批量重试可能要等很久，没有进度条会重新变成用户反馈过的"点不动、用不了"体验（见"v1.3.10"一节）。
- 不要把原生超时错误（`"原生功能调用超时（xxx），请重试"`）加进 `isTransientApiError()`/`retryTransient()` 的自动重试范围——故意不做，那套机制只认 HTTP 400；一次原生超时本身就已经等了 15 分钟，再自动重试 N 次会让单张图片失败前的等待时间成倍增加（见"v1.3.10"一节"没有做但要知道的事"）。

## 下一步建议

- **最优先**：找用户确认 v1.3.10 的两处修复是否真的解决了"总是原生调用超时"和"全部失败重试点不动"——这是用户明确报告"用不了"的功能性问题，比其它"体验是否符合预期"类的待确认项更紧迫。具体确认：(1) 15 分钟的新超时是否还会在正常使用中触发（如果还触发，说明要么 15 分钟依然不够，要么存在真正的原生调用挂起，需要考虑加 AbortSignal 取消支持而不是继续调大数字）；(2) "全部失败重试"点击后进度条是否正常显示、体感是否不再像"卡死"。
