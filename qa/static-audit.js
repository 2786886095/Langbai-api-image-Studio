...
assert.equal(version, "1.3.27", "APP_VERSION must be the release source of truth");
assert.match(pubspec, /^version:\s*1\.3\.27\+51$/m);
assert.match(html, /v1\.3\.27/);
assert.match(html, /20260724-1-3-27/g);
assert.match(sw, /ai-image-generator-1-3-27-20260724/);
assert.match(sw, /ignoreSearch:\s*true/);
assert.match(runnerRc, /VERSION_AS_NUMBER 1,3,27,51/);
assert.match(runnerRc, /VERSION_AS_STRING "1\.3\.27"/);
