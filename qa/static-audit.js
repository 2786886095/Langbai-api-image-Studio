...
assert.equal(version, "1.5.3", "APP_VERSION must be the release source of truth");
assert.match(pubspec, /^version:\s*1\.5\.3\+75$/m);
...
assert.match(html, /v1\.5\.3/);
assert.match(html, /20260729-1-5-3/g);
assert.match(sw, /ai-image-generator-1-5-3-20260729/);
assert.match(sw, /codex-image-gateway\.js/);
assert.match(sw, /ignoreSearch:\s*true/);
assert.match(runnerRc, /VERSION_AS_NUMBER 1,5,3,75/);
assert.match(runnerRc, /VERSION_AS_STRING "1\.5\.3"/);
assert.match(workflow, /const APP_VERSION = "1\.5\.3";/);
assert.match(workflow, /bootstrap-guard\.js\\\?v=20260729-1-5-3/);
