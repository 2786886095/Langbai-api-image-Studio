...
assert.equal(version, "1.4.6", "APP_VERSION must be the release source of truth");
assert.match(pubspec, /^version:\s*1\.4\.6\+68$/m);
...
assert.match(html, /20260728-1-4-6/g);
assert.match(sw, /ai-image-generator-1-4-6-20260728/);
assert.match(sw, /codex-image-gateway\.js/);
assert.match(sw, /ignoreSearch:\s*true/);
assert.match(runnerRc, /VERSION_AS_NUMBER 1,4,6,68/);
assert.match(runnerRc, /VERSION_AS_STRING "1\.4\.6"/);
assert.match(workflow, /const APP_VERSION = "1\.4\.6";/);
assert.match(workflow, /bootstrap-guard\.js\\\?v=20260728-1-4-6/);
