...
assert.equal(version, "1.5.2", "APP_VERSION must be the release source of truth");
assert.match(pubspec, /^version:\s*1\.5\.2\+74$/m);
...
assert.match(html, /20260729-1-5-2/g);
assert.match(sw, /ai-image-generator-1-5-2-20260729/);
assert.match(sw, /codex-image-gateway\.js/);
assert.match(sw, /ignoreSearch:\s*true/);
assert.match(runnerRc, /VERSION_AS_NUMBER 1,5,2,74/);
assert.match(runnerRc, /VERSION_AS_STRING "1\.5\.2"/);
assert.match(workflow, /const APP_VERSION = "1\.5\.2";/);
assert.match(workflow, /bootstrap-guard\.js\\\?v=20260729-1-5-2/);
