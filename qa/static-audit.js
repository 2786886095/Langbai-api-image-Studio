...
const geminiWatermarkRemover = read("gemini-watermark-remover.bundle.js");
...
const embeddedGatewayImageTasks = read("embedded_gateway/vendor/chatgpt2api/services/image_task_service.py");
const embeddedGatewayBrowserImages = read("embedded_gateway/vendor/chatgpt2api/services/browser_image_service.py");
...
const windowsMain = read("windows/runner/main.cpp");
const windowsInstaller = read("windows/installer/setup.iss");

const expectedVersion = "1.7.1";
const expectedBuild = 114;
const expectedCacheToken = "20260910-1-7-1";
const expectedSwCache = "ai-image-generator-1-7-1-20260910-startup";
...
assert.match(pubspec, /^\s*- gemini-watermark-remover\.bundle\.js$/m);
...
assert.match(html, /20260910-1-7-1/g);
assert.match(sw, /ai-image-generator-1-7-1-20260910/);
const localCacheTokens = [
  ...html.matchAll(/(?:href|src)="(?:\.\/)?(?:[a-z0-9-]+\.(?:css|js))\?v=([^"]+)"/gi),
].map(match => match[1]);
...
assert.match(sw, /gemini-watermark-remover\.bundle\.js/);
assert.match(geminiWatermarkRemover, /@pilio\/gemini-watermark-remover v1\.0\.33/);
assert.match(geminiWatermarkRemover, /processWatermarkBlob/);
assert.ok(
  html.indexOf("gemini-watermark-remover.bundle.js") < html.indexOf("app.js?v="),
  "Watermark remover bundle must load before app.js",
);
assert.match(app, /geminiWatermarkRemovalEnabled:\s*true/);
assert.match(app, /async function removeGeminiWatermarkBlob\(blob\)/);
...
assert.match(runnerRc, /VERSION_AS_NUMBER 1,7,1,114/);
assert.match(runnerRc, /VERSION_AS_STRING "1\.7\.1"/);
assert.match(workflow, /const APP_VERSION = "1\.7\.1";/);
assert.match(workflow, /bootstrap-guard\.js\\\?v=20260910-1-7-1/);
...
assert.match(workflow, /node --check gemini-watermark-remover\.bundle\.js/);
assert.match(workflow, /"gemini-watermark-remover\.bundle\.js"/);
...
assert.match(releaseNotes, new RegExp(`^# .*v${expectedVersion.replaceAll(".", "\\.")}$`, "m"));
assert.match(releaseNotes, new RegExp(`${expectedVersion.replaceAll(".", "\\.")}\\+${expectedBuild}`));
...
assert.match(androidChatGptGateway, /default_concurrency': 5/);
...
assert.match(androidGatewayPython, /client\.delete_conversation\(\)/);
assert.match(androidGatewayPython, /"is_visible": False/);
assert.match(embeddedGatewayImageTasks, /TASK_STATUS_CANCELLED = "cancelled"/);
assert.match(embeddedGatewayImageTasks, /min\(100, max_concurrency\)/);
assert.match(embeddedGatewayBrowserImages, /backend\.delete_conversation\(conversation_id\)/);
assert.match(embeddedChatGptGateway, /SessionActivationCoordinator/);
assert.match(embeddedChatGptGateway, /LANGBAI_IMAGE_MAX_CONCURRENCY': '100'/);
assert.match(dartMain, /_chatGptAccountActivations/);
assert.match(dartMain, /_silentlyRefreshChatGptAccount/);
assert.match(dartMain, /--chatgpt-auth-silent/);
assert.match(windowsMain, /silent_chatgpt_auth/);
assert.match(windowsMain, /FlutterWindow window\(project, !silent_chatgpt_auth\)/);
assert.match(vendoredWindowsPlugin, /LangbaiSilentWindow/);
assert.match(app, /session_refreshed/);
assert.match(chatGptMultiAccount, /expiryCandidates/);
...
assert.match(html, /id="projectZipInput"[^>]*accept="\.zip/);
assert.match(app, /async function extractProjectZipFiles\(zipFile\)/);
assert.match(app, /new DecompressionStream\("deflate-raw"\)/);
assert.match(app, /PROJECT_IMPORT_MAX_TOTAL_BYTES/);
assert.match(app, /async function restoreExportedProjectFromZip\(zipFile\)/);
...
assert.match(
  app,
  /geminiWatermarkRemovalEnabled === false[\s\S]*?blob,[\s\S]*?final:\s*source,[\s\S]*?transform:\s*"none"/,
  "Disabling Gemini watermark removal must preserve the downloaded blob",
);
assert.match(
  app,
  /const removed = await removeGeminiWatermarkBlob\(blob\)[\s\S]*?blob:\s*removed\.blob[\s\S]*?transform:\s*"gemini_watermark_removed"/,
  "Enabled Gemini watermark removal must expose only the processed blob",
);
...
assert.match(codexImageGateway, /CLIENT_MAX_CONCURRENCY\s*=\s*100/);
assert.match(codexImageGateway, /clientQueue:\s*5/);
assert.match(html, /id="codexGatewayClientQueue"[^>]*min="1"[^>]*max="100"[^>]*value="5"/);
...
assert.match(imageTaskStability, /status === 422[\s\S]*?category = ERROR_CATEGORIES\.parameters;[\s\S]*?retryPolicy = "edit_required"/);
assert.match(imageTaskStability, /status === 400[\s\S]*?category = ERROR_CATEGORIES\.unknown;[\s\S]*?retryPolicy = "manual_limited"/);
