...
assert.equal(version, "1.5.1", "APP_VERSION must be the release source of truth");
assert.match(pubspec, /^version:\s*1\.5\.1\+73$/m);
...
assert.match(html, /20260729-1-5-1/g);
assert.match(sw, /ai-image-generator-1-5-1-20260729/);
assert.match(sw, /codex-image-gateway\.js/);
assert.match(sw, /ignoreSearch:\s*true/);
assert.match(runnerRc, /VERSION_AS_NUMBER 1,5,1,73/);
assert.match(runnerRc, /VERSION_AS_STRING "1\.5\.1"/);
assert.match(workflow, /const APP_VERSION = "1\.5\.1";/);
assert.match(workflow, /bootstrap-guard\.js\\\?v=20260729-1-5-1/);
