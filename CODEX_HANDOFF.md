...
- 全部失败重试的用户授权轮次不再被 `retryDirective()` 提前终止，审核、鉴权、参数、任务丢失与未知结果 504 均执行完整轮次。
- 瞬时错误继续采用结构化退避；其它错误在轮次之间至少等待 1.5 秒，成功图片立即退出队列。
- 重试运行新增 active/completed/failed/succeeded 集合，避免补充操作重复提交正在运行或排队的卡片。
- “补充剩余失败”可重启已耗尽卡片并重置其轮次，同时修正 done/ok/failed 聚合计数；未跟踪失败也会加入。
- 浏览器完整回归覆盖审核多轮、504 多轮、耗尽重启与遗漏收集；Android 内嵌资源同步，Gemini 去水印保持通过。
...
- 通用 HTTP 400 失败改为 `manual_limited`，可以进入单卡和全部失败重试；明确参数、审核、认证失败继续隔离。
- 全部失败重试显示可重试数/失败总数；全为阻断项时定位并聚焦第一个待编辑卡片。
- Windows/macOS/Android/iOS 深浅色 Headless 审计覆盖主界面、分镜表和设置弹窗；所有视口页面横向溢出与可见控件越界均为 0。
- 分镜/三视图表格不再隐藏重试、参考图和删除操作；Android 内置资源已同步。
- Node 单元测试 24 项、完整浏览器回归、静态审计、`flutter analyze` 和 Flutter 81 项测试通过；打包进程测试按环境跳过 1 项。
...
- 新增技能管理器，技能按功能类/风格类分组，支持自定义名称、提示词模板及单图/漫画/两者作用范围。
- 单图和漫画分镜分别持久化勾选状态，提示词按技能列表顺序注入；内置赛璐璐商业漫画风格技能默认关闭。
- 原嵌字模式及嵌字编辑、批量嵌字代码已移除，第三模式替换为批量角色三视图生成。
- 每张导入图片作为独立角色参考并允许填写独立说明；每行单独请求，固定 16:9 白底正面、90°侧面、背面全身三视图。
- 三视图不使用单图或漫画技能，支持历史、失败重试、项目导出与恢复。
- JavaScript/网关/Service Worker 静态测试、完整浏览器回归、`flutter analyze` 与 Flutter 全量测试已通过。
...
- ChatGPT 网页生图网关保留健康检查的实际失败原因；可恢复故障会自动重启网关并复检。
- 同一批并发 502/503 只累计一次；熔断期间显示秒级倒计时，到期后自动健康检测。
