...
class WebviewController extends ValueNotifier<WebviewValue> {
  ...
  Future<void> _setPointerButtonState(
      PointerButton button, bool isDown, Offset position) async {
    if (_isDisposed) {
      return;
    }
    assert(value.isInitialized);
  ...
  Future<void> _setScrollDelta(double dx, double dy, Offset position) async {
    if (_isDisposed) {
      return;
    }
    assert(value.isInitialized);
    return _methodChannel
        .invokeMethod('setScrollDelta', [dx, dy, position.dx, position.dy]);
  }
...
class _WebviewState extends State<Webview> {
                ...
                ? Listener(
                    ...
                    onPointerSignal: (signal) {
                      if (signal is PointerScrollEvent) {
                        _controller._setScrollDelta(-signal.scrollDelta.dx,
                            -signal.scrollDelta.dy, signal.localPosition);
                      }
                    },
                    onPointerPanZoomUpdate: (signal) {
                      if (signal.panDelta.dx.abs() > signal.panDelta.dy.abs()) {
                        _controller._setScrollDelta(
                            -signal.panDelta.dx, 0, signal.localPosition);
                      } else {
                        _controller._setScrollDelta(
                            0, signal.panDelta.dy, signal.localPosition);
                      }
                    },
                    child: MouseRegion(
                        cursor: _cursor,
  ...
  void _reportSurfaceSize() async {
    ...
    if (box != null) {
      final surfaceSize = box.size;
      if (surfaceSize.width < 2 || surfaceSize.height < 2) {
        return;
      }
      await _controller.ready;
      unawaited(_controller._setSize(surfaceSize,
