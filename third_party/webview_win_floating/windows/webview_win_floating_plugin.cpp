...
void WebviewWinFloatingPlugin::createWebview(const flutter::MethodCall<flutter::EncodableValue> &method_call,
  ...
  params.onProcessFailed = [=](int kind) -> void {
    ...
    m_MethodChannel->InvokeMethod("onProcessFailed", std::make_unique<flutter::EncodableValue>(arguments));
  };
...
void WebviewWinFloatingPlugin::HandleMethodCall(
  ...
  if (isCreateCall) {
    ...
    if (FAILED(hr)) shared_result->Error("runJavascript() error");
  } else if (method_call.method_name().compare("addScriptToExecuteOnDocumentCreated") == 0) {
    std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> shared_result = std::move(result);
    auto javaScriptString = std::get<std::string>(arguments[flutter::EncodableValue("javaScriptString")]);
    webview->addScriptToExecuteOnDocumentCreated(
      utf8ToUtf16(javaScriptString).c_str(),
      [shared_result](HRESULT hr) {
        if (FAILED(hr)) shared_result->Error("addScriptToExecuteOnDocumentCreated() error");
        else shared_result->Success();
      });
    ...
    if (FAILED(hr)) result->Error("addScriptChannelByName() error");
    else result->Success();
