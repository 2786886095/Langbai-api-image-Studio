...
from services import browser_image_service
...
def _public_task(task: dict[str, Any]) -> dict[str, Any]:
    ...
    if task.get("langbai") is not None:
        item["langbai"] = task.get("langbai")
    if task.get("error"):
        item["error"] = task.get("error")
        item["error_status"] = task.get("error_status") or 502
        item["error_type"] = task.get("error_type") or "api_error"
        item["error_code"] = task.get("error_code") or "upstream_error"
...
class ImageTaskService:
    def __init__(
        ...
        path: Path,
        *,
        generation_handler: Callable[[dict[str, Any]], dict[str, Any]] = browser_image_service.handle_generation,
        edit_handler: Callable[[dict[str, Any]], dict[str, Any]] = browser_image_service.handle_edit,
        retention_days_getter: Callable[[], int] | None = None,
        concurrency_getter: Callable[[], int] | None = None,
    ):
        self.path = path
        self.generation_handler = generation_handler
        self.edit_handler = edit_handler
        self.retention_days_getter = retention_days_getter or (lambda: config.image_retention_days)
        self.concurrency_getter = concurrency_getter or (lambda: config.image_account_concurrency)
        try:
    ...
    def _run_task_in_slot(
        ...
        try:
            ...
            error_status = int(getattr(exc, "status_code", 502) or 502)
            error_type = _clean(getattr(exc, "error_type", "api_error"), "api_error")
            error_code = _clean(getattr(exc, "code", "upstream_error"), "upstream_error")
            account_email = _clean(getattr(exc, "account_email", ""))
            conversation_id = _clean(getattr(exc, "conversation_id", ""))
            duration_ms = int((time.time() - started) * 1000)
            self._update_task(key, status=TASK_STATUS_ERROR, error=error_message,
    ...
    def _load_locked(self) -> dict[str, dict[str, Any]]:
        ...
        for item in raw_items:
            ...
            if error:
                task["error"] = error
                task["error_status"] = int(item.get("error_status") or 502)
                task["error_type"] = _clean(item.get("error_type"), "api_error")
                task["error_code"] = _clean(item.get("error_code"), "upstream_error")
