...
def build_image_prompt(prompt: str, size: str | None, quality: str = "auto") -> str:
    ...
    if size:
        ...
        try:
            ...
            if width > height:
                orientation = "横屏构图，禁止输出竖屏或交换宽高"
            elif height > width:
                orientation = "竖屏构图，禁止输出横屏或交换宽高"
            else:
                orientation = "正方形构图"
        except Exception:
            orientation = ""
        hints.append(f"【画布硬性要求】目标尺寸 {size}；{orientation}。")
    if quality:
        hints.append(f"输出质量档位：{quality}。")
    return f"{''.join(hints)}\n\n{prompt.strip()}" if hints else prompt
